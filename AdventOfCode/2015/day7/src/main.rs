...
fn solve_part_one(input: &str) -> i32 {
    ...
    return calculate_variable("a", &variables_equations, &mut already_calculated_variables, &mut operations);
}
...
fn solve_part_two(input: &str) -> i32{
    ...
    println!("... Finished preparing variables equations");
    let first_variable_a: i32 = calculate_variable("a", &variables_equations, &mut already_calculated_variables, &mut operations);
...
fn calculate_variable<'a>(variable: &'a str,variables_equations: &HashMap<&'a str, Vec<&'a str>>, already_calculated_variables: &mut HashMap<&'a str, i32>, operations: &mut Vec<&'a str>) -> i32{
    ...
    while !already_calculated_variables.contains_key(variable) {
        ...
        match equation.len() {
            ...
            1 => {
                if !already_calculated_variables.get(equation.get(0).unwrap()).is_none(){
                    already_calculated_variables.insert(operations.last().unwrap(), *already_calculated_variables.get(equation.get(0).unwrap()).unwrap());
                    operations.pop();
            ...
            2 => {
                if !already_calculated_variables.get(equation.get(1).unwrap()).is_none(){
                    already_calculated_variables.insert(operations.last().unwrap(), bitwise_not(*already_calculated_variables.get(equation.get(1).unwrap()).unwrap()));
                    operations.pop();
                } else if let Ok(value) = equation.get(1).unwrap().parse::<i32>() {
                    already_calculated_variables.insert(operations.last().unwrap(), bitwise_not(value));
                    operations.pop();
                }else{
            ...
            3 => {
                let first_param: Option<i32> = {
                    if let Ok(value) = equation.get(0).unwrap().parse::<i32>() {
                        Some(value)
                    }else if !already_calculated_variables.get(equation.get(0).unwrap()).is_none(){
                        Some(*already_calculated_variables.get(equation.get(0).unwrap()).unwrap())
                    }else{
                        operations.push(equation.get(0).unwrap());
                        None
                    }
                ...
                let second_param: Option<i32> = {
                    if let Ok(value) = equation.get(2).unwrap().parse::<i32>() {
                        Some(value)
                    }else if !already_calculated_variables.get(equation.get(2).unwrap()).is_none(){
                        Some(*already_calculated_variables.get(equation.get(2).unwrap()).unwrap())
                    }else{
                        operations.push(equation.get(2).unwrap());
                        None
                    }
                };
                if !first_param.is_none() && !second_param.is_none(){
                    let mut calculated_value: i32 = 0;
                    match *equation.get(1).unwrap(){
                        "OR" => calculated_value = bitwise_or(first_param.unwrap(), second_param.unwrap()),
                        "AND" => calculated_value = bitwise_and(first_param.unwrap(), second_param.unwrap()),
                        "RSHIFT" => calculated_value = bitwise_rshift(first_param.unwrap(), second_param.unwrap()),
                        "LSHIFT" => calculated_value = bitwise_lshift(first_param.unwrap(), second_param.unwrap()),
                        _ =>{}
                    }
                    already_calculated_variables.insert(operations.last().unwrap(), calculated_value);
                    operations.pop();
                }
            }
            _ => {}
