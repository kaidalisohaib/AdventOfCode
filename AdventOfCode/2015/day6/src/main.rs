...
use std::{collections::{HashSet, HashMap}, fs, process};
...
fn solve_part_one(input: &str) -> usize{
    ...
    let reg: Regex = Regex::new(r"^(\w+) (?:(\w+) )?(\d{1,3}),(\d{1,3}) through (\d{1,3}),(\d{1,3})$").unwrap();
    for line in input.lines() {
        let groups: Captures = reg.captures(line).unwrap();
        let start: (u16, u16) = (groups.get(3).unwrap().as_str().parse().unwrap(), groups.get(4).unwrap().as_str().parse().unwrap()); 
        let end  : (u16, u16) = (groups.get(5).unwrap().as_str().parse().unwrap(), groups.get(6).unwrap().as_str().parse().unwrap()); 
